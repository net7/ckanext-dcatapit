...
def get_frequency_url(frequency_value):
    """
    Costruisce l'URL della frequenza basandosi sul valore fornito.
    
    Args:
        frequency_value: Può essere una stringa (codice della frequenza), 
                        una lista di stringhe, o un dizionario con 'value'
    
    Returns:
        URL completo nel formato: http://publications.europa.eu/resource/authority/frequency/{CODICE}
        Se il valore è già un URL, lo restituisce così com'è
        Se il valore è una lista, restituisce il primo URL valido
    """
    from ckanext.dcatapit.dcat.const import FREQ_BASE_URI
    ...
    if isinstance(frequency_value, str):
        if frequency_value.startswith('http://') or frequency_value.startswith('https://'):
            return frequency_value
        # Estrai il codice dalla stringa (potrebbe essere nel formato "{CODE}" o "CODE")
        code = frequency_value.strip('{}').strip()
        if code:
            return f"{FREQ_BASE_URI}{code}"
    ...
    if isinstance(frequency_value, list):
        ...
        first_value = frequency_value[0]
        # Se è un dizionario con 'value', usa quello
        if isinstance(first_value, dict):
            code = first_value.get('value', '')
        else:
            code = first_value
