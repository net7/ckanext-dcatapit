...
#    plugins.implements(plugins.IRoutes, inherit=True)
    ...
    def get_helpers(self):
        dcatapit_helpers = {
            ...
            'today_as_string': helpers.today_as_string,
            ...
            'get_organization_name_and_identifier': helpers.get_organization_name_and_identifier,
        }
    ...
    def before_dataset_index(self, dataset_dict):
       ...
       # theme_normal = dataset_dict.get(tuple['theme'])
        ...
        # Gestione sicura del parsing JSON del theme
        try:
            if dataset_dict.get('theme') and json.loads(dataset_dict['theme']):
            ...
            log.warning('Error parsing theme JSON for dataset %s: %s', dataset_dict.get('id', 'unknown'), str(e))
            dataset_dict['theme'] = 'OP_DATPRO'
            theme_normal1 = dataset_dict['theme']
...
#        aggr_themes = helpers.dcatapit_string_to_aggregated_themes(extra_theme)
        ...
        # Controllo di sicurezza per aggr_themes
        if aggr_themes is None:
            aggr_themes = []
        
        # Controllo che tutti gli elementi siano dizionari validi
        aggr_themes = [t for t in aggr_themes if t and isinstance(t, dict) and 'theme' in t]
...
#        aggr_theme1=str(aggr_themes).replace('http://publications.europa.eu/resource/authority/data-theme/','')
        ...
        if extra_theme.find('theme')>0:
         search_terms = [t.get('theme') for t in aggr_themes if t and isinstance(t, dict)]
  #       search_terms=list(theme_normal)
         ...
         try: 
          for t in aggr_themes:
           ...
           log.warning('Error processing aggr_themes: %s', str(e))
           stringtheme=''
...
#        if len(extra_theme)>4:
         for t in aggr_themes:
            if t and isinstance(t, dict):
                ...
                if subthemes and isinstance(subthemes, (list, tuple)):
                    search_subthemes.extend(subthemes)
        except Exception as e:
            log.warning('Error processing subthemes for dataset %s: %s', dataset_dict.get('id', 'unknown'), str(e))
        if search_subthemes:
            dataset_dict['dcat_subtheme'] = search_subthemes
            try:
                localized_subthemes = interfaces.get_localized_subthemes(search_subthemes)
                if localized_subthemes and isinstance(localized_subthemes, dict):
                    for lang, subthemes in localized_subthemes.items():
                        if subthemes and isinstance(subthemes, (list, tuple)):
                            dataset_dict['dcat_subtheme_{}'.format(lang)] = subthemes
            except Exception as e:
                log.warning('Error processing localized subthemes for dataset %s: %s', dataset_dict.get('id', 'unknown'), str(e))
        
        # Gestione sicura del parsing JSON del data_dict
        try:
            ddict = json.loads(dataset_dict['data_dict'])
        except (ValueError, TypeError, json.JSONDecodeError) as e:
            log.warning('Error parsing data_dict for dataset %s: %s', dataset_dict.get('id', 'unknown'), str(e))
            ddict = {}
        ...
        if org_id:
            try:
                ...
                log.warning('Error getting organization %s for dataset %s: %s', org_id, dataset_dict.get('id', 'unknown'), str(e))
                org = {}
        else:
            org = {}
        
        if org and isinstance(org, dict) and org.get('region'):
            ...
            for region_name in region_base:
                ...
                if ltags and isinstance(ltags, dict):
